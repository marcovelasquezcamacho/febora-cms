...
const tipoColors: Record<string, { bg: string; border: string; badge: string }> = {
  info:        { bg: 'rgba(30,80,180,0.15)',  border: 'rgba(30,80,180,0.35)',  badge: '#7EA8E5' },
  alerta:      { bg: 'rgba(200,16,46,0.12)',  border: 'rgba(200,16,46,0.35)',  badge: '#ff7a85' },
  invitacion:  { bg: 'rgba(0,122,51,0.12)',   border: 'rgba(0,122,51,0.35)',   badge: '#4CAF7D' },
  comunicado:  { bg: 'rgba(242,169,0,0.12)',  border: 'rgba(242,169,0,0.35)', badge: '#F2A900' },
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* IMAGEN — sin recorte, muestra completa */}
        {popup.imagen_url && (
          <div style={{ width: '100%', background: '#0D0D0D', display: 'flex', alignItems: 'center', justifyContent: 'center', maxHeight: '320px', overflow: 'hidden' }}>
            <img
        ...
        <div style={{ padding: '1.75rem' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1rem', gap: '1rem' }}>
            <div>
              <span style={{
                ...
                border: `1px solid ${colors.border}`, marginBottom: '0.65rem'
              }}>
                {tipoLabels[popup.tipo] || popup.tipo}
              </span>
              <h2 style={{
                fontFamily: "'Bebas Neue', serif", fontSize: '1.8rem',
                letterSpacing: '0.04em', lineHeight: 1.05, color: '#F0EEE8', margin: 0
              }}>{popup.titulo}</h2>
            </div>
          ...
          {popup.contenido && (
            <p style={{
              ...
              lineHeight: 1.75, marginBottom: popup.btn_label ? '1.5rem' : '0', margin: '0 0 1.25rem'
            }}>{popup.contenido}</p>
          )}
        ...
        <div style={{
          ...
          fontFamily: "'Bebas Neue', serif", fontSize: '0.85rem',
          letterSpacing: '0.12em', color: 'rgba(255,255,255,0.08)',
          textAlign: 'right'
        }}>FE<span style={{ color: 'rgba(242,169,0,0.15)' }}>BORA</span></div>
      </div>
    </div>
  )
}
